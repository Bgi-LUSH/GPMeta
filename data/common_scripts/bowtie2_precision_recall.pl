...
while(<IN2>){
	...
	#my $chr=$a[2];
        ...
        my $ASscore;
        if($a[11]=~/AS:i:(\d+)/){$ASscore=$1;}else{$ASscore="0";}
	#my $mapq=$a[1]

	if(!exists $readhash{$a[0]}){
		#if (($a[1]&0x4) || $a[3] eq '*'){$total_unmapread;};
		$readhash{$a[0]}="$a[2];$ASscore;$a[4]";
		#$total_mapread++;
	}else{
		$readhash{$a[0]}.="\t$a[2];$ASscore;$a[4]";
	}
...
for(keys %readhash){
	...
	for my $t(0..$#acc){
		my $chr_list=$acc[$t];
		my($chr,$as,$mapq)=split/;/,$chr_list; 
		#if($chr eq "*"){$star_num++;next;}  #unmaped read

                if($t==0){
                        @best_reads=($chr,$as,$mapq);
			@best_chr=($chr);
                }else{
                        if($as > $best_reads[1] || ($as == $best_reads[1] && $mapq > $best_reads[2])){
                                @best_reads=($chr,$as,$mapq);
				@best_chr=($chr);
                        }elsif($as == $best_reads[1] && $mapq == $best_reads[2]){
				push @best_chr,$chr;
			}else{;}
                }
	}
