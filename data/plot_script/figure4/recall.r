...
setwd("D:/工作/2020.4-华大科技股份/病原快测/文章撰写和分析/作图/真实样本")
...
for(i in gro){
  ...
  D <- ddply(d, .(group),transform, pos = cumsum(recall)) #- (1 * recall))
  p1<-ggplot(D,aes(x=group,y=recall,fill=group)) + guides(fill=FALSE)+
  ...
  ggtitle(i)+theme(plot.title = element_text(hjust = "0.5",face="bold"))+
	...
	geom_text(data=D,aes(x=group,y=pos,label=recall),size=3,color="black")
